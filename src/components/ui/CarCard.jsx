...
function CarCard({ Car }) {
  return (
    <div className=' hover:shadow-lg cursor-pointer rounded-b-xl'>
      <img src={Car.carImages.imageURL} alt="" width={300} height={250} className='rounded-t-xl mt-10'/>
      <div className='p-4'>
        <h2 className='font-bold text-sm md:text-lg mb-2'>{Car.carList.listingTitle}</h2>
        ...
        <div className='grid grid-rows-3 sm:grid-rows-none md:grid-cols-3 mt-5 mb-5'>
            <div className="flex flex-col item-center">
                ...
                <h4>{Car.carList.transmission}</h4>
            </div>
            <div className="flex flex-col item-center">
                <IoLogoModelS className='text-xl md:text-2xl'/>
                <h4>{Car.carList.model}</h4>
            </div>
            <div className="flex flex-col item-center">
                <BsFuelPumpDieselFill  className='text-xl md:text-2xl'/>
                <h4>{Car.carList.fuelType}</h4>
            </div>
        </div>
        <Separator className='border-2'/>
        <div className='flex flex-row justify-between my-5'>
            <h4 className='font-bold tex-md md:text-lg'>₱{Car.carList.originalPrice} / hour</h4>
